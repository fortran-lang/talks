...
\begin{frame}{Building blocks for Fortran implementation}
   \begin{columns}
      ...
      \begin{column}{.40\textwidth}
         \begin{enumerate}
            \item command-line interface builder
            \item TOML parser library
            \item Fortran source scanning
            \item multiprocessing and scheduling
         \end{enumerate}
      \end{column}
      \begin{column}{.55\textwidth}
         \begin{itemize}
            \item[\alert{▶}] \alert{M\_CLI2} package
            \item[\alert{▶}] \alert{toml-f} package
            \item[\alert{▶}] implemented in \alert{fpm}
            \item[\alert{▶}] \alert{OpenMP} directives
         \end{itemize}
      \end{column}
   \end{columns}
\end{frame}
